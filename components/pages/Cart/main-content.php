<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
</head>
<body>
<div class="cart-page__contents__main__wrap">

<div class="indi-cartitem">

    <div class="indi-cartitem__left">
        <div class="indi-cartitem__left__image">
            <img src="./assets/images/spinach.jpg"/>
        </div>
        <div class="indi-cartitem__left__details">
           
            <div class="indi-cartitem__left__details__price mobile-view">

                Rs.1000
            </div>

         <div>
            <div class="indi-cartitem__left__details__name">
                Spinach Krantikari
            </div>
            <div class="indi-cartitem__left__details__vendor">
                <span class="indi-cartitem__left__details__vendor__by">Sold By :</span> GreenGrocesssors
            </div>
            </div>

            <div class="indi-cartitem__left__details__other-details mobile-view">
            <div class="indi-cartitem__right__others__mobile-functions">


            <div class="indi-cartitem__right__quantity">
            
            <div class="indi-cartitem__right__quantity__button">
                    <!-- Minus button -->
                    <a 
                    href="cart.php?product_id=4&qty=<?php  $quantity = $_SESSION['cart'][4];echo "$quantity";?>&type=subs">
                <div class="indi-cartitem__right__quantity__button__change">
                       
                        <svg width="10" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.5833 3.08334H0.416626V0.916672H15.5833V3.08334Z" fill="currentColor"/>
                        </svg>
                        
                </div>
                </a>  

                <div class="indi-cartitem__right__quantity__button__num">
                        <?php
                        echo "$quantity";
                        ?>
                </div> 
                
                    <!-- Plus button -->
                    <a 
                    href="cart.php?product_id=4&qty=<?php  $quantity = $_SESSION['cart'][4];echo "$quantity";?>&type=subs">
                <div class="indi-cartitem__right__quantity__button__change">
                
                        <svg width="10" height="10" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.7832 9.08333H9.28325V15.5833H7.11658V9.08333H0.616577V6.91667H7.11658V0.416664H9.28325V6.91667H15.7832V9.08333Z" fill="currentColor"/>
                        </svg>
                        
                </div> 
                </a>
            </div>
        </div>

<div class="indi-cartitem__left__details__other-details__functions">

<!-- Add to wishlist  -->
<div class="indi-cartitem__right__others__functions__icon">
    <svg width="22" height="22" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M9 16.0125L7.9125 15.0225C4.05 11.52 1.5 9.21 1.5 6.375C1.5 4.065 3.315 2.25 5.625 2.25C6.93 2.25 8.1825 2.8575 9 3.8175C9.8175 2.8575 11.07 2.25 12.375 2.25C14.685 2.25 16.5 4.065 16.5 6.375C16.5 9.21 13.95 11.52 10.0875 15.03L9 16.0125Z" fill="#B12525"/>
    </svg>
    
</div>

 <!-- Delete from cart -->
 <a  href="cart.php?product_id=4&qty=<?php  $quantity = $_SESSION['cart'][4];echo "$quantity";?>&type=delete"> 
<div class="indi-cartitem__right__others__functions__icon">
    <svg width="14" height="20" viewBox="0 0 18 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M1.5 20.75C1.5 22.125 2.625 23.25 4 23.25H14C15.375 23.25 16.5 22.125 16.5 20.75V5.75H1.5V20.75ZM17.75 2H13.375L12.125 0.75H5.875L4.625 2H0.25V4.5H17.75V2Z" fill="currentColor"/>
    </svg>
    
</div>
</a>
</div>


</div>
                    
            </div>
            
       
       
        </div>
    </div>

    <div class="indi-cartitem__right desktop-view">
        <div class="indi-cartitem__right__others">
            <div class="indi-cartitem__right__others__price">
                Rs.100
            </div>
            <div class="indi-cartitem__right__others__functions">

                    <!-- Add to wishlist  -->
                    <a 
                    href="cart.php?product_id=4&qty=<?php  $quantity = $_SESSION['cart'][4];echo "$quantity";?>&type=subs">
                <div class="indi-cartitem__right__others__functions__icon">
                        <svg width="22" height="22" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 16.0125L7.9125 15.0225C4.05 11.52 1.5 9.21 1.5 6.375C1.5 4.065 3.315 2.25 5.625 2.25C6.93 2.25 8.1825 2.8575 9 3.8175C9.8175 2.8575 11.07 2.25 12.375 2.25C14.685 2.25 16.5 4.065 16.5 6.375C16.5 9.21 13.95 11.52 10.0875 15.03L9 16.0125Z" fill="#B12525"/>
                        </svg>
                        
                </div>

                     <!-- Delete from cart -->
                     <a 
                    href="cart.php?product_id=4&qty=<?php  $quantity = $_SESSION['cart'][4];echo "$quantity";?>&type=delete">
                <div class="indi-cartitem__right__others__functions__icon">
                        <svg width="14" height="20" viewBox="0 0 18 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M1.5 20.75C1.5 22.125 2.625 23.25 4 23.25H14C15.375 23.25 16.5 22.125 16.5 20.75V5.75H1.5V20.75ZM17.75 2H13.375L12.125 0.75H5.875L4.625 2H0.25V4.5H17.75V2Z" fill="currentColor"/>
                        </svg>
                        
                </div>
                </a>
                
            </div>
        </div>

        <div class="indi-cartitem__right__quantity">
            <div class="indi-cartitem__right__quantity__label">
                Qty :
            </div>
            <div class="indi-cartitem__right__quantity__button">
                    <!-- Minus button -->
                    <a 
                    href="cart.php?product_id=4&qty=<?php  $quantity = $_SESSION['cart'][4];echo "$quantity";?>&type=subs">
                <div class="indi-cartitem__right__quantity__button__change">
                        <svg width="10" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.5833 3.08334H0.416626V0.916672H15.5833V3.08334Z" fill="currentColor"/>
                        </svg>
                        
                </div>  
                </a>

                <div class="indi-cartitem__right__quantity__button__num">
                        <?php echo"$quantity"?>
                </div> 
                
                    <!-- Plus button -->
                    <a 
                    href="cart.php?product_id=4&qty=<?php  $quantity = $_SESSION['cart'][4];echo "$quantity";?>&type=add">
                <div class="indi-cartitem__right__quantity__button__change">
                        <svg width="10" height="10" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.7832 9.08333H9.28325V15.5833H7.11658V9.08333H0.616577V6.91667H7.11658V0.416664H9.28325V6.91667H15.7832V9.08333Z" fill="currentColor"/>
                        </svg>
                        
                </div> 
                </a>
            </div>
        </div>
    </div>

</div>


<div class="indi-cartitem">

    <div class="indi-cartitem__left">
        <div class="indi-cartitem__left__image">
            <img src="./assets/images/spinach.jpg"/>
        </div>
        <div class="indi-cartitem__left__details">
           
            <div class="indi-cartitem__left__details__price mobile-view">

                Rs.1000
            </div>

         <div>
            <div class="indi-cartitem__left__details__name">
                Spinach Krantikari
            </div>
            <div class="indi-cartitem__left__details__vendor">
                <span class="indi-cartitem__left__details__vendor__by">Sold By :</span> GreenGrocesssors
            </div>
            </div>

            <div class="indi-cartitem__left__details__other-details mobile-view">
            <div class="indi-cartitem__right__others__mobile-functions">


            <div class="indi-cartitem__right__quantity">
            
            <div class="indi-cartitem__right__quantity__button">
                    <!-- Minus button -->
                    <a 
                    href="cart.php?product_id=1&qty=<?php  $quantity = $_SESSION['cart'][1];echo "$quantity";?>&type=subs">
                <div class="indi-cartitem__right__quantity__button__change">
                       
                        <svg width="10" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.5833 3.08334H0.416626V0.916672H15.5833V3.08334Z" fill="currentColor"/>
                        </svg>
                        
                </div>
                </a>  

                <div class="indi-cartitem__right__quantity__button__num">
                        <?php
                        echo "$quantity";
                        ?>
                </div> 
                
                    <!-- Plus button -->
                    <a 
                    href="cart.php?product_id=1&qty=<?php  $quantity = $_SESSION['cart'][1];echo "$quantity";?>&type=subs">
                <div class="indi-cartitem__right__quantity__button__change">
                
                        <svg width="10" height="10" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.7832 9.08333H9.28325V15.5833H7.11658V9.08333H0.616577V6.91667H7.11658V0.416664H9.28325V6.91667H15.7832V9.08333Z" fill="currentColor"/>
                        </svg>
                        
                </div> 
                </a>
            </div>
        </div>

<div class="indi-cartitem__left__details__other-details__functions">

<!-- Add to wishlist  -->
<div class="indi-cartitem__right__others__functions__icon">
    <svg width="22" height="22" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M9 16.0125L7.9125 15.0225C4.05 11.52 1.5 9.21 1.5 6.375C1.5 4.065 3.315 2.25 5.625 2.25C6.93 2.25 8.1825 2.8575 9 3.8175C9.8175 2.8575 11.07 2.25 12.375 2.25C14.685 2.25 16.5 4.065 16.5 6.375C16.5 9.21 13.95 11.52 10.0875 15.03L9 16.0125Z" fill="#B12525"/>
    </svg>
    
</div>

 <!-- Delete from cart -->
 <a  href="cart.php?product_id=1&qty=<?php  $quantity = $_SESSION['cart'][1];echo "$quantity";?>&type=delete"> 
<div class="indi-cartitem__right__others__functions__icon">
    <svg width="14" height="20" viewBox="0 0 18 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M1.5 20.75C1.5 22.125 2.625 23.25 4 23.25H14C15.375 23.25 16.5 22.125 16.5 20.75V5.75H1.5V20.75ZM17.75 2H13.375L12.125 0.75H5.875L4.625 2H0.25V4.5H17.75V2Z" fill="currentColor"/>
    </svg>
    
</div>
</a>
</div>


</div>
                    
            </div>
            
       
       
        </div>
    </div>

    <div class="indi-cartitem__right desktop-view">
        <div class="indi-cartitem__right__others">
            <div class="indi-cartitem__right__others__price">
                Rs.100
            </div>
            <div class="indi-cartitem__right__others__functions">

                    <!-- Add to wishlist  -->
                    <a 
                    href="cart.php?product_id=1&qty=<?php  $quantity = $_SESSION['cart'][1];echo "$quantity";?>&type=subs">
                <div class="indi-cartitem__right__others__functions__icon">
                        <svg width="22" height="22" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 16.0125L7.9125 15.0225C4.05 11.52 1.5 9.21 1.5 6.375C1.5 4.065 3.315 2.25 5.625 2.25C6.93 2.25 8.1825 2.8575 9 3.8175C9.8175 2.8575 11.07 2.25 12.375 2.25C14.685 2.25 16.5 4.065 16.5 6.375C16.5 9.21 13.95 11.52 10.0875 15.03L9 16.0125Z" fill="#B12525"/>
                        </svg>
                        
                </div>

                     <!-- Delete from cart -->
                     <a 
                    href="cart.php?product_id=1&qty=<?php  $quantity = $_SESSION['cart'][1];echo "$quantity";?>&type=delete">
                <div class="indi-cartitem__right__others__functions__icon">
                        <svg width="14" height="20" viewBox="0 0 18 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M1.5 20.75C1.5 22.125 2.625 23.25 4 23.25H14C15.375 23.25 16.5 22.125 16.5 20.75V5.75H1.5V20.75ZM17.75 2H13.375L12.125 0.75H5.875L4.625 2H0.25V4.5H17.75V2Z" fill="currentColor"/>
                        </svg>
                        
                </div>
                </a>
                
            </div>
        </div>

        <div class="indi-cartitem__right__quantity">
            <div class="indi-cartitem__right__quantity__label">
                Qty :
            </div>
            <div class="indi-cartitem__right__quantity__button">
                    <!-- Minus button -->
                    <a 
                    href="cart.php?product_id=1&qty=<?php  $quantity = $_SESSION['cart'][1];echo "$quantity";?>&type=subs">
                <div class="indi-cartitem__right__quantity__button__change">
                        <svg width="10" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.5833 3.08334H0.416626V0.916672H15.5833V3.08334Z" fill="currentColor"/>
                        </svg>
                        
                </div>  
                </a>

                <div class="indi-cartitem__right__quantity__button__num">
                        <?php echo"$quantity"?>
                </div> 
                
                    <!-- Plus button -->
                    <a 
                    href="cart.php?product_id=1&qty=<?php  $quantity = $_SESSION['cart'][1];echo "$quantity";?>&type=add">
                <div class="indi-cartitem__right__quantity__button__change">
                        <svg width="10" height="10" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.7832 9.08333H9.28325V15.5833H7.11658V9.08333H0.616577V6.91667H7.11658V0.416664H9.28325V6.91667H15.7832V9.08333Z" fill="currentColor"/>
                        </svg>
                        
                </div> 
                </a>
            </div>
        </div>
    </div>

</div>







<div class="indi-cartitem">

    <div class="indi-cartitem__left">
        <div class="indi-cartitem__left__image">
            <img src="./assets/images/spinach.jpg"/>
        </div>
        <div class="indi-cartitem__left__details">
           
            <div class="indi-cartitem__left__details__price mobile-view">

                Rs.1000
            </div>

         <div>
            <div class="indi-cartitem__left__details__name">
                Spinach Krantikari
            </div>
            <div class="indi-cartitem__left__details__vendor">
                <span class="indi-cartitem__left__details__vendor__by">Sold By :</span> GreenGrocesssors
            </div>
            </div>

            <div class="indi-cartitem__left__details__other-details mobile-view">
            <div class="indi-cartitem__right__others__mobile-functions">


            <div class="indi-cartitem__right__quantity">
            
            <div class="indi-cartitem__right__quantity__button">
                    <!-- Minus button -->
                <div class="indi-cartitem__right__quantity__button__change">
                        <svg width="10" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.5833 3.08334H0.416626V0.916672H15.5833V3.08334Z" fill="currentColor"/>
                        </svg>
                        
                </div>  

                <div class="indi-cartitem__right__quantity__button__num">
                        7
                </div> 
                
                    <!-- Plus button -->
                <div class="indi-cartitem__right__quantity__button__change">
                        <svg width="10" height="10" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.7832 9.08333H9.28325V15.5833H7.11658V9.08333H0.616577V6.91667H7.11658V0.416664H9.28325V6.91667H15.7832V9.08333Z" fill="currentColor"/>
                        </svg>
                        
                </div> 
            </div>
        </div>

<div class="indi-cartitem__left__details__other-details__functions">

<!-- Add to wishlist  -->
<div class="indi-cartitem__right__others__functions__icon">
    <svg width="22" height="22" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M9 16.0125L7.9125 15.0225C4.05 11.52 1.5 9.21 1.5 6.375C1.5 4.065 3.315 2.25 5.625 2.25C6.93 2.25 8.1825 2.8575 9 3.8175C9.8175 2.8575 11.07 2.25 12.375 2.25C14.685 2.25 16.5 4.065 16.5 6.375C16.5 9.21 13.95 11.52 10.0875 15.03L9 16.0125Z" fill="#B12525"/>
    </svg>
    
</div>

 <!-- Delete from cart -->
<div class="indi-cartitem__right__others__functions__icon">
    <svg width="14" height="20" viewBox="0 0 18 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M1.5 20.75C1.5 22.125 2.625 23.25 4 23.25H14C15.375 23.25 16.5 22.125 16.5 20.75V5.75H1.5V20.75ZM17.75 2H13.375L12.125 0.75H5.875L4.625 2H0.25V4.5H17.75V2Z" fill="currentColor"/>
    </svg>
    
</div>
</div>


</div>
                    
            </div>
            
       
       
        </div>
    </div>

    <div class="indi-cartitem__right desktop-view">
        <div class="indi-cartitem__right__others">
            <div class="indi-cartitem__right__others__price">
                Rs.100
            </div>
            <div class="indi-cartitem__right__others__functions">

                    <!-- Add to wishlist  -->
                <div class="indi-cartitem__right__others__functions__icon">
                        <svg width="22" height="22" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 16.0125L7.9125 15.0225C4.05 11.52 1.5 9.21 1.5 6.375C1.5 4.065 3.315 2.25 5.625 2.25C6.93 2.25 8.1825 2.8575 9 3.8175C9.8175 2.8575 11.07 2.25 12.375 2.25C14.685 2.25 16.5 4.065 16.5 6.375C16.5 9.21 13.95 11.52 10.0875 15.03L9 16.0125Z" fill="#B12525"/>
                        </svg>
                        
                </div>

                     <!-- Delete from cart -->
                <div class="indi-cartitem__right__others__functions__icon">
                        <svg width="14" height="20" viewBox="0 0 18 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M1.5 20.75C1.5 22.125 2.625 23.25 4 23.25H14C15.375 23.25 16.5 22.125 16.5 20.75V5.75H1.5V20.75ZM17.75 2H13.375L12.125 0.75H5.875L4.625 2H0.25V4.5H17.75V2Z" fill="currentColor"/>
                        </svg>
                        
                </div>
                
            </div>
        </div>

        <div class="indi-cartitem__right__quantity">
            <div class="indi-cartitem__right__quantity__label">
                Qty :
            </div>
            <div class="indi-cartitem__right__quantity__button">
                    <!-- Minus button -->
                <div class="indi-cartitem__right__quantity__button__change">
                        <svg width="10" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.5833 3.08334H0.416626V0.916672H15.5833V3.08334Z" fill="currentColor"/>
                        </svg>
                        
                </div>  

                <div class="indi-cartitem__right__quantity__button__num">
                        7
                </div> 
                
                    <!-- Plus button -->
                <div class="indi-cartitem__right__quantity__button__change">
                        <svg width="10" height="10" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.7832 9.08333H9.28325V15.5833H7.11658V9.08333H0.616577V6.91667H7.11658V0.416664H9.28325V6.91667H15.7832V9.08333Z" fill="currentColor"/>
                        </svg>
                        
                </div> 
            </div>
        </div>
    </div>

</div>
<!-- <div class="indi-cartitem">

    <div class="indi-cartitem__left">
        <div class="indi-cartitem__left__image">
            <img src="./assets/images/spinach.jpg"/>
        </div>
        <div class="indi-cartitem__left__details">
            <div class="indi-cartitem__left__details__name">
                Spinach Krantikari
            </div>
            <div class="indi-cartitem__left__details__vendor">
                <span class="indi-cartitem__left__details__vendor__by">Sold By :</span> GreenGrocesssors
            </div>
        </div>
    </div>

    <div class="indi-cartitem__right desktop-view">
        <div class="indi-cartitem__right__others">
            <div class="indi-cartitem__right__others__price">
                Rs.100
            </div>
            <div class="indi-cartitem__right__others__functions">

                    <!-- Add to wishlist  
                <div class="indi-cartitem__right__others__functions__icon">
                        <svg width="22" height="22" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 16.0125L7.9125 15.0225C4.05 11.52 1.5 9.21 1.5 6.375C1.5 4.065 3.315 2.25 5.625 2.25C6.93 2.25 8.1825 2.8575 9 3.8175C9.8175 2.8575 11.07 2.25 12.375 2.25C14.685 2.25 16.5 4.065 16.5 6.375C16.5 9.21 13.95 11.52 10.0875 15.03L9 16.0125Z" fill="#B12525"/>
                        </svg>
                        
                </div>

                     <!-- Delete from cart 
                <div class="indi-cartitem__right__others__functions__icon">
                        <svg width="14" height="20" viewBox="0 0 18 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M1.5 20.75C1.5 22.125 2.625 23.25 4 23.25H14C15.375 23.25 16.5 22.125 16.5 20.75V5.75H1.5V20.75ZM17.75 2H13.375L12.125 0.75H5.875L4.625 2H0.25V4.5H17.75V2Z" fill="currentColor"/>
                        </svg>
                        
                </div>
                
            </div>
        </div>

        <div class="indi-cartitem__right__quantity">
            <div class="indi-cartitem__right__quantity__label">
                Qty :
            </div>
            <div class="indi-cartitem__right__quantity__button">
                    <!-- Minus button 
                <div class="indi-cartitem__right__quantity__button__change">
                        <svg width="10" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.5833 3.08334H0.416626V0.916672H15.5833V3.08334Z" fill="currentColor"/>
                        </svg>
                        
                </div>  

                <div class="indi-cartitem__right__quantity__button__num">
                        7
                </div> 
                
                    <!-- Plus button 
                <div class="indi-cartitem__right__quantity__button__change">
                        <svg width="10" height="10" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M15.7832 9.08333H9.28325V15.5833H7.11658V9.08333H0.616577V6.91667H7.11658V0.416664H9.28325V6.91667H15.7832V9.08333Z" fill="currentColor"/>
                        </svg>
                        
                </div> 
            </div>
        </div>
    </div>

</div> -->

</div>
</body>
</html>